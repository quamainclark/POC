...
export class AppComponent implements OnInit {
  ...
  ngOnInit() {
    console.log("***");
    // this.roleService.getCurrentRole();
    // console.log("*****");
      ...
      if (this.currentRole === "host") {
        ...
        this.http.get<any>(`${url}/todos/1`).subscribe(serverItems => {
          serverItems.content.map(serverItem => {
            ...
            item.name = serverItem[7].split(".")[0];
            item.price = serverItem[2];
            item.description = serverItem[1];
    ...
    // const currentRole: string = this.roleService.getRole();
